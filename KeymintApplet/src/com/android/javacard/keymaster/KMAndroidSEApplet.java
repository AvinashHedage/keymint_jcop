...
public class KMAndroidSEApplet extends KMKeymasterApplet implements OnUpgradeListener {
  ...
  private static final byte ILLEGAL_STATE = KM_BEGIN_STATE + 1;
	...
	/*
	 * private static final byte ATT_ID_BRAND = 0; private static final byte
	 * ATT_ID_DEVICE = 1; private static final byte ATT_ID_PRODUCT = 2; private
	 * static final byte ATT_ID_SERIAL = 3; private static final byte ATT_ID_IMEI =
	 * 4; private static final byte ATT_ID_MEID = 5; private static final byte
	 * ATT_ID_MANUFACTURER = 6; private static final byte ATT_ID_MODEL = 7;
	 */
  ...
  private void processProvisionAttestIdsCmd(APDU apdu) {
	  ...
	  ((KMAndroidSEProvider)seProvider).setAttestationIds(attData);
		/*
		 * saveAttId((short)KMType.ATTESTATION_ID_BRAND);
		 * saveAttId((short)KMType.ATTESTATION_ID_IMEI);
		 * saveAttId((short)KMType.ATTESTATION_ID_DEVICE);
		 * saveAttId((short)KMType.ATTESTATION_ID_MEID);
		 * saveAttId((short)KMType.ATTESTATION_ID_MODEL);
		 * saveAttId((short)KMType.ATTESTATION_ID_MANUFACTURER);
		 * saveAttId((short)KMType.ATTESTATION_ID_PRODUCT);
		 * saveAttId((short)KMType.ATTESTATION_ID_SERIAL);
		 */
  }
  
	/*
	 * private void saveAttId(short attTag) { short keyParms =
	 * KMKeyParameters.findTag(KMType.BYTES_TAG, attTag, data[KEY_PARAMETERS]); if
	 * (keyParms != KMType.INVALID_VALUE) { keyParms =
	 * KMByteTag.cast(keyParms).getValue();
	 * ((KMAndroidSEProvider)seProvider).setAttestationId(mapToAttId(attTag),
	 * KMByteBlob.cast(keyParms).getBuffer(),
	 * KMByteBlob.cast(keyParms).getStartOff(), KMByteBlob.cast(keyParms).length());
	 * } else { KMException.throwIt(KMError.INVALID_ARGUMENT); } }
	 */

	/*
	 * private byte mapToAttId(short attTag) { switch (attTag) { case
	 * KMType.ATTESTATION_ID_BRAND: return ATT_ID_BRAND; case
	 * KMType.ATTESTATION_ID_DEVICE: return ATT_ID_DEVICE; case
	 * KMType.ATTESTATION_ID_IMEI: return ATT_ID_IMEI; case
	 * KMType.ATTESTATION_ID_MANUFACTURER: return ATT_ID_MANUFACTURER; case
	 * KMType.ATTESTATION_ID_MEID: return ATT_ID_MEID; case
	 * KMType.ATTESTATION_ID_MODEL: return ATT_ID_MODEL; case
	 * KMType.ATTESTATION_ID_PRODUCT: return ATT_ID_PRODUCT; case
	 * KMType.ATTESTATION_ID_SERIAL: return ATT_ID_SERIAL; }
	 * KMException.throwIt(KMError.INVALID_TAG); return (byte) 0xFF; // should never
	 * happen }
	 */
...
//This function masks the error code with POWER_RESET_MASK_FLAG
 ...
 private static short buildErrorStatus(short err) {
  ...
  private boolean isProvisioningComplete() {
	    if ((0 != (provisionStatus & PROVISION_STATUS_ATTEST_IDS))
