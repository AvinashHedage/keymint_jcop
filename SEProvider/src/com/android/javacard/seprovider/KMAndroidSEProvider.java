...
public class KMAndroidSEProvider implements KMSEProvider {
	...
	final byte[] CIPHER_ALGS = { Cipher.ALG_AES_BLOCK_128_CBC_NOPAD, Cipher.ALG_AES_BLOCK_128_ECB_NOPAD,
			Cipher.ALG_DES_CBC_NOPAD, Cipher.ALG_DES_ECB_NOPAD, Cipher.ALG_AES_CTR, Cipher.ALG_RSA_PKCS1,
			KMRsaOAEPEncoding.ALG_RSA_PKCS1_OAEP_SHA256_MGF1_SHA1, Cipher.ALG_RSA_NOPAD, AEADCipher.ALG_AES_GCM };

	final byte[] SIG_ALGS = { Signature.ALG_RSA_SHA_256_PKCS1, Signature.ALG_RSA_SHA_256_PKCS1_PSS,
			Signature.ALG_ECDSA_SHA_256, Signature.ALG_HMAC_SHA_256, KMRsa2048NoDigestSignature.ALG_RSA_SIGN_NOPAD,
			KMRsa2048NoDigestSignature.ALG_RSA_PKCS1_NODIGEST, KMEcdsa256NoDigestSignature.ALG_ECDSA_NODIGEST };
	...
	public static void initStatics() {
		secp256r1_P = new byte[] { (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0x00, (byte) 0x00,
		...
		secp256r1_A = new byte[] { (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0x00, (byte) 0x00,
				(byte) 0x00, (byte) 0x01, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00,
				(byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0xFF, (byte) 0xFF,
				(byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF,
				(byte) 0xFF, (byte) 0xFC };

		secp256r1_B = new byte[] { (byte) 0x5A, (byte) 0xC6, (byte) 0x35, (byte) 0xD8, (byte) 0xAA, (byte) 0x3A,
		...
		secp256r1_S = new byte[] { (byte) 0xC4, (byte) 0x9D, (byte) 0x36, (byte) 0x08, (byte) 0x86, (byte) 0xE7,
				(byte) 0x04, (byte) 0x93, (byte) 0x6A, (byte) 0x66, (byte) 0x78, (byte) 0xE1, (byte) 0x13, (byte) 0x9D,
				(byte) 0x26, (byte) 0xB7, (byte) 0x81, (byte) 0x9F, (byte) 0x7E, (byte) 0x90 };

		// Uncompressed form
		secp256r1_UCG = new byte[] { (byte) 0x04, (byte) 0x6B, (byte) 0x17, (byte) 0xD1, (byte) 0xF2, (byte) 0xE1,
		...
		secp256r1_N = new byte[] { (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0xFF, (byte) 0x00, (byte) 0x00,
	...
	public KMAndroidSEProvider() {
		...
		aesKeys[KEYSIZE_128_OFFSET] = (AESKey) KeyBuilder.buildKey(KeyBuilder.TYPE_AES, KeyBuilder.LENGTH_AES_128,
				false);
		aesKeys[KEYSIZE_256_OFFSET] = (AESKey) KeyBuilder.buildKey(KeyBuilder.TYPE_AES, KeyBuilder.LENGTH_AES_256,
				false);
		triDesKey = (DESKey) KeyBuilder.buildKey(KeyBuilder.TYPE_DES, KeyBuilder.LENGTH_DES3_3KEY, false);
		hmacKey = (HMACKey) KeyBuilder.buildKey(KeyBuilder.TYPE_HMAC, (short) 512, false);
		...
		resetFlag = JCSystem.makeTransientByteArray((short) 1, JCSystem.CLEAR_ON_DESELECT);// new byte[1];
	...
	public RSAPrivateKey createRsaKey(byte[] modBuffer, short modOff, short modLength, byte[] privBuffer, short privOff,
			short privLength) {
	...
	public void createAsymmetricKey(byte alg, byte[] privKeyBuf, short privKeyStart, short privKeyLength,
		...
		case KMType.RSA:
			...
			Util.arrayCopyNonAtomic(tmpArray, (short) 0, privKeyBuf,
					(short) (privKeyStart + privKeyLength - lengths[0]), lengths[0]);
			// Copy modulus
			Util.arrayFillNonAtomic(tmpArray, (short) 0, RSA_KEY_SIZE, (byte) 0);
			...
			Util.arrayCopyNonAtomic(tmpArray, (short) 0, pubModBuf, (short) (pubModStart + pubModLength - lengths[1]),
					lengths[1]);
			break;
		case KMType.EC:
	...
	public short aesGCMEncrypt(AESKey key, byte[] secret, short secretStart, short secretLen, byte[] encSecret,
			short encSecretStart, byte[] nonce, short nonceStart, short nonceLen, byte[] authData, short authDataStart,
	...
	@Override
	public short aesGCMEncrypt(byte[] aesKey, short aesKeyStart, short aesKeyLen, byte[] secret, short secretStart,
			short secretLen, byte[] encSecret, short encSecretStart, byte[] nonce, short nonceStart, short nonceLen,
		...
		return aesGCMEncrypt(key, secret, secretStart, secretLen, encSecret, encSecretStart, nonce, nonceStart,
				nonceLen, authData, authDataStart, authDataLen, authTag, authTagStart, authTagLen);
	}

	@Override
	public boolean aesGCMDecrypt(byte[] aesKey, short aesKeyStart, short aesKeyLen, byte[] encSecret,
			short encSecretStart, short encSecretLen, byte[] secret, short secretStart, byte[] nonce, short nonceStart,
			short nonceLen, byte[] authData, short authDataStart, short authDataLen, byte[] authTag, short authTagStart,
			short authTagLen) {
		if (aesGcmCipher == null) {
			aesGcmCipher = (AEADCipher) Cipher.getInstance(AEADCipher.ALG_AES_GCM, false);
		}
		...
		verification = aesGcmCipher.verifyTag(authTag, authTagStart, (short) authTagLen, (short) AES_GCM_TAG_LENGTH);
		return verification;
	}

	public HMACKey cmacKdf(KMPreSharedKey preSharedKey, byte[] label, short labelStart, short labelLen, byte[] context,
			short contextStart, short contextLength) {
		try {
			...
			while (i <= n) {
				...
				pos = kdf.sign(L, (short) 0, (short) 4, tmpArray, (short) (iBufLen + pos));
				i++;
			}
	...
	public boolean hmacVerify(HMACKey key, byte[] data, short dataStart, short dataLength, byte[] mac, short macStart,
			short macLength) {
	...
	public short hmacKDF(KMMasterKey masterkey, byte[] data, short dataStart, short dataLength, byte[] signature,
		...
		try {
			...
			return hmacSign(keyData, (short) 0, keyLen, data, dataStart, dataLength, signature, signatureStart);
		} finally {
			clean();
		}
	}
	...
	@Override
	public short rsaDecipherOAEP256(byte[] secret, short secretStart, short secretLength, byte[] modBuffer,
		...
		return rsaOaepDecipher.doFinal(inputDataBuf, (short) inputDataStart, (short) inputDataLength, outputDataBuf,
	...
	public Cipher createSymmetricCipher(short alg, short purpose, short blockMode, short padding, byte[] secret,
			short secretStart, short secretLength, byte[] ivBuffer, short ivStart, short ivLength) {
		Key key = null;
		...
		case AEADCipher.ALG_AES_GCM:
			((AEADCipher) symmCipher).init(key, mapPurpose(purpose), ivBuffer, ivStart, ivLength);
			break;
	...
	public Signature createHmacSignerVerifier(short purpose, short digest, byte[] secret, short secretStart,
			short secretLength) {
	...
	public KMOperation initSymmetricOperation(byte purpose, byte alg, byte digest, byte padding, byte blockMode,
		...
		case KMType.AES:
		case KMType.DES:
			Cipher cipher = createSymmetricCipher(alg, purpose, blockMode, padding, keyBuf, keyStart, keyLength, ivBuf,
					ivStart, ivLength);
	...
	public Signature createRsaSigner(short digest, short padding, byte[] secret, short secretStart, short secretLength,
			byte[] modBuffer, short modOff, short modLength) {
		byte alg = mapSignature256Alg(KMType.RSA, (byte) padding, (byte) digest);
		byte opMode;
		if (padding == KMType.PADDING_NONE || (padding == KMType.RSA_PKCS1_1_5_SIGN && digest == KMType.DIGEST_NONE)) {
	...
	public Cipher createRsaDecipher(short padding, short digest, byte[] secret, short secretStart, short secretLength,
			byte[] modBuffer, short modOff, short modLength) {
	...
	@Override
	public KMOperation initAsymmetricOperation(byte purpose, byte alg, byte padding, byte digest, byte mgfDigest,
			byte[] privKeyBuf, short privKeyStart, short privKeyLength, byte[] pubModBuf, short pubModStart,
			short pubModLength) {
		KMOperationImpl opr = null;
		if (alg == KMType.RSA) {
			switch (purpose) {
			case KMType.SIGN:
				Signature signer = createRsaSigner(digest, padding, privKeyBuf, privKeyStart, privKeyLength, pubModBuf,
				...
				break;
			case KMType.DECRYPT:
				Cipher decipher = createRsaDecipher(padding, mgfDigest, privKeyBuf, privKeyStart, privKeyLength,
						pubModBuf, pubModStart, pubModLength);
				opr = getOperationInstanceFromPool();
	...
	@Override
	public short cmacKDF(KMPreSharedKey pSharedKey, byte[] label, short labelStart, short labelLen, byte[] context,
			short contextStart, short contextLength, byte[] keyBuf, short keyStart) {
		HMACKey key = cmacKdf(pSharedKey, label, labelStart, labelLen, context, contextStart, contextLength);
	...
	public void persistPartialCertificateChain(byte[] buf, short offset, short len, short totalLen) {
		...
		Util.arrayCopyNonAtomic(buf, offset, certificateChain, (short) (persistedLen + 2), len);
		JCSystem.commitTransaction();
	}
	...
	public short getBackupObjectCount() {
		short count = (short) (1 /* Certificate chain */ + KMAESKey.getBackupObjectCount()
				+ KMECPrivateKey.getBackupObjectCount() + KMHmacKey.getBackupObjectCount());
		return count;
	}
	...
	@Override
	public short ecSign256(byte[] secret, short secretStart, short secretLength, byte[] inputDataBuf,
			short inputDataStart, short inputDataLength, byte[] outputDataBuf, short outputDataStart) {
	...
	@Override
	public short rsaSign256Pkcs1(byte[] secret, short secretStart, short secretLength, byte[] modBuf, short modStart,
			short modLength, byte[] inputDataBuf, short inputDataStart, short inputDataLength, byte[] outputDataBuf,
			short outputDataStart) {

		Signature.OneShot signer = null;
		try {

			signer = Signature.OneShot.open(MessageDigest.ALG_SHA_256, Signature.SIG_CIPHER_RSA, Cipher.PAD_PKCS1);
	...
	public void setAttestationIds(short attIdVals) {
		...
		while (index < length) {
			...
			// Attestation Id Brand
			case KMType.ATTESTATION_ID_BRAND:
				attIdBrand = new byte[KMByteBlob.cast(obj).length()];
				Util.arrayCopyNonAtomic(KMByteBlob.cast(obj).getBuffer(), KMByteBlob.cast(obj).getStartOff(),
						attIdBrand, (short) 0, KMByteBlob.cast(obj).length());
				break;
			// Attestation Id Device
			case KMType.ATTESTATION_ID_DEVICE:
				attIdDevice = new byte[KMByteBlob.cast(obj).length()];
				Util.arrayCopyNonAtomic(KMByteBlob.cast(obj).getBuffer(), KMByteBlob.cast(obj).getStartOff(),
						attIdDevice, (short) 0, KMByteBlob.cast(obj).length());
				break;
			// Attestation Id Product
			case KMType.ATTESTATION_ID_PRODUCT:
				attIdProduct = new byte[KMByteBlob.cast(obj).length()];
				Util.arrayCopyNonAtomic(KMByteBlob.cast(obj).getBuffer(), KMByteBlob.cast(obj).getStartOff(),
						attIdProduct, (short) 0, KMByteBlob.cast(obj).length());
				break;
			// Attestation Id Serial
			case KMType.ATTESTATION_ID_SERIAL:
				attIdSerial = new byte[KMByteBlob.cast(obj).length()];
				Util.arrayCopyNonAtomic(KMByteBlob.cast(obj).getBuffer(), KMByteBlob.cast(obj).getStartOff(),
						attIdSerial, (short) 0, KMByteBlob.cast(obj).length());
				break;
			// Attestation Id IMEI
			case KMType.ATTESTATION_ID_IMEI:
				attIdImei = new byte[KMByteBlob.cast(obj).length()];
				Util.arrayCopyNonAtomic(KMByteBlob.cast(obj).getBuffer(), KMByteBlob.cast(obj).getStartOff(), attIdImei,
						(short) 0, KMByteBlob.cast(obj).length());
				break;
			// Attestation Id MEID
			case KMType.ATTESTATION_ID_MEID:
				attIdMeId = new byte[KMByteBlob.cast(obj).length()];
				Util.arrayCopyNonAtomic(KMByteBlob.cast(obj).getBuffer(), KMByteBlob.cast(obj).getStartOff(), attIdMeId,
						(short) 0, KMByteBlob.cast(obj).length());
				break;
			// Attestation Id Manufacturer
			case KMType.ATTESTATION_ID_MANUFACTURER:
				attIdManufacturer = new byte[KMByteBlob.cast(obj).length()];
				Util.arrayCopyNonAtomic(KMByteBlob.cast(obj).getBuffer(), KMByteBlob.cast(obj).getStartOff(),
						attIdManufacturer, (short) 0, KMByteBlob.cast(obj).length());
				break;
			// Attestation Id Model
			case KMType.ATTESTATION_ID_MODEL:
				attIdModel = new byte[KMByteBlob.cast(obj).length()];
				Util.arrayCopyNonAtomic(KMByteBlob.cast(obj).getBuffer(), KMByteBlob.cast(obj).getStartOff(),
						attIdModel, (short) 0, KMByteBlob.cast(obj).length());
				break;
			}
			index++;
		}
	...
	@Override
	public short hkdf(byte[] ikm, short ikmOff, short ikmLen, byte[] salt, short saltOff, short saltLen, byte[] info,
			short infoOff, short infoLen, byte[] out, short outOff, short outLen) {
		// HMAC_extract
	...
	@Override
	public short ecdhKeyAgreement(byte[] privKey, short privKeyOff, short privKeyLen, byte[] publicKey,
			short publicKeyOff, short publicKeyLen, byte[] secret, short secretOff) {
	...
	public boolean ecVerify256(byte[] pubKey, short pubKeyOffset, short pubKeyLen, byte[] inputDataBuf,
			short inputDataStart, short inputDataLength, byte[] signatureDataBuf, short signatureDataStart,
		...
		try {
			...
			return signer.verify(inputDataBuf, inputDataStart, inputDataLength, signatureDataBuf, signatureDataStart,
	...
	@Override
	public short ecSign256(KMDeviceUniqueKey ecPrivKey, byte[] inputDataBuf, short inputDataStart,
	...
	private KMDeviceUniqueKey createDeviceUniqueKey(KMECDeviceUniqueKey key, byte[] pubKey, short pubKeyOff,
	...
	public KMDeviceUniqueKey createDeviceUniqueKey(boolean testMode, byte[] pubKey, short pubKeyOff, short pubKeyLen,
		...
		if (testMode) {
			key = createDeviceUniqueKey(testKey, pubKey, pubKeyOff, pubKeyLen, privKey, privKeyOff, privKeyLen);
			if (testKey == null)
				testKey = (KMECDeviceUniqueKey) key;
		} else {
			key = createDeviceUniqueKey(deviceUniqueKey, pubKey, pubKeyOff, pubKeyLen, privKey, privKeyOff, privKeyLen);
	...
	public byte[] getAdditionalCertChain() {
		...
		// Util.arrayCopyNonAtomic(additionalCertChain, (short) 2, buffer, start,
		// length);
		// return length;
	}

	/*
	 * @Override public short generateBcc(boolean testMode, byte[] scratchPad) { if
	 * (!testMode && isProvisionLocked) {
	 * KMException.throwIt(KMError.STATUS_FAILED); } KMDeviceUniqueKey
	 * deviceUniqueKey = getDeviceUniqueKey(testMode); short temp =
	 * deviceUniqueKey.getPublicKey(scratchPad, (short) 0); short coseKey =
	 * KMCose.constructCoseKey( KMInteger.uint_8(KMCose.COSE_KEY_TYPE_EC2),
	 * KMType.INVALID_VALUE, KMNInteger.uint_8(KMCose.COSE_ALG_ES256),
	 * KMInteger.uint_8(KMCose.COSE_KEY_OP_VERIFY),
	 * KMInteger.uint_8(KMCose.COSE_ECCURVE_256), scratchPad, (short) 0, temp,
	 * KMType.INVALID_VALUE, false ); //TODO
	 * 
	 * temp = KMKeymasterApplet.encodeToApduBuffer(coseKey, scratchPad, (short) 0,
	 * KMKeymasterApplet.MAX_COSE_BUF_SIZE);
	 * 
	 * // Construct payload. short payload = KMCose.constructCoseCertPayload(
	 * KMCosePairTextStringTag.instance(KMInteger.uint_8(KMCose.ISSUER),
	 * KMTextString.instance(KMCose.TEST_ISSUER_NAME, (short) 0, (short)
	 * KMCose.TEST_ISSUER_NAME.length)),
	 * KMCosePairTextStringTag.instance(KMInteger.uint_8(KMCose.SUBJECT),
	 * KMTextString.instance(KMCose.TEST_SUBJECT_NAME, (short) 0, (short)
	 * KMCose.TEST_SUBJECT_NAME.length)),
	 * KMCosePairByteBlobTag.instance(KMNInteger.uint_32(KMCose.SUBJECT_PUBLIC_KEY,
	 * (short) 0), KMByteBlob.instance(scratchPad, (short) 0, temp)),
	 * KMCosePairByteBlobTag.instance(KMNInteger.uint_32(KMCose.KEY_USAGE, (short)
	 * 0), KMByteBlob.instance(KMCose.KEY_USAGE_SIGN, (short) 0, (short)
	 * KMCose.KEY_USAGE_SIGN.length)) ); // temp temporarily holds the length of
	 * encoded cert payload. //TODO
	 * 
	 ...
	 * 
	 * // protected header short protectedHeader =
	 * KMCose.constructHeaders(KMNInteger.uint_8(KMCose.COSE_ALG_ES256),
	 * KMType.INVALID_VALUE, KMType.INVALID_VALUE, KMType.INVALID_VALUE); // temp
	 * temporarily holds the length of encoded headers. //TODO
	 * 
	 ...
	 * 
	 * //unprotected headers. short arr = KMArray.instance((short) 0); short
	 * unprotectedHeader = KMCoseHeaders.instance(arr);
	 * 
	 * // construct cose sign structure. short coseSignStructure =
	 * KMCose.constructCoseSignStructure(protectedHeader,
	 * KMByteBlob.instance((short) 0), payload); // temp temporarily holds the
	 * length of encoded sign structure. // Encode cose Sign_Structure. //TODO
	 * 
	 * temp = KMKeymasterApplet.encodeToApduBuffer(coseSignStructure, scratchPad,
	 * (short) 0, KMKeymasterApplet.MAX_COSE_BUF_SIZE);
	 * 
	 * // do sign short len = ecSign256( deviceUniqueKey, scratchPad, (short) 0,
	 * temp, scratchPad, temp ); coseSignStructure = KMByteBlob.instance(scratchPad,
	 * temp, len);
	 * 
	 * // construct cose_sign1 short coseSign1 =
	 * KMCose.constructCoseSign1(protectedHeader, unprotectedHeader, payload,
	 * coseSignStructure);
	 * 
	 * // [Cose_Key, Cose_Sign1] short bcc = KMArray.instance((short) 2);
	 * KMArray.cast(bcc).add((short) 0, coseKey); KMArray.cast(bcc).add((short) 1,
	 * coseSign1); return bcc; }
	 */
